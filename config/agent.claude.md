...
- Create goals using `python -m src.goals.cli set-goal ...` for each yearly and monthly goal.
...
- Record the result using `python -m src.goals.cli record ...`
...
1. **Read everything:** Profile + `python -m src.goals.cli summary --period <week>` + `python -m src.goals.cli history --days 7`
...
```bash
# Create a goal
python -m src.goals.cli set-goal --title "..." --why "..." --timeframe yearly --period 2026

# Create a monthly sub-goal linked to parent
python -m src.goals.cli set-goal --title "..." --why "..." --timeframe monthly --period 2026-03 --parent-goal-id <uuid>

# Update a goal's status
python -m src.goals.cli update-goal --goal-id <uuid> --status completed

# Record daily outcome
python -m src.goals.cli record --goal-id <uuid> --status completed --reason "Finished the chapter"

# List active goals
python -m src.goals.cli list
python -m src.goals.cli list --timeframe monthly --period 2026-03

# Get recent history
python -m src.goals.cli history --days 7
python -m src.goals.cli history --goal-id <uuid>

# Get period summary (completion rates, streaks, skip reasons)
python -m src.goals.cli summary --period 2026-03
