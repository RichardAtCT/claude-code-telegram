...
**Goal**: Prevent session hijacking and replay attacks.

1. **Session binding**: Bind Claude sessions to (user_id, device_fingerprint, IP range) tuple. Reject session resume from different context.

2. **Session token rotation**: After each interaction, rotate the session token stored locally. The Claude session ID itself doesn't change, but our mapping to it does.

3. **Session expiry enforcement**: Current 24-hour timeout is good. Add hard limit of 7 days regardless of activity.

4. **Invalidation**: `/new` command should cryptographically destroy the old session key material (if using Layer 3 encryption), not just set a flag.
...
```
  ...
  src/claude/facade.py            - Session binding
```

Addresses: Memory exposure, session hijacking.
...
| Layer | Protects Against | Effort | Dependencies | Breaking Change |
|---|---|---|---|---|
...
| 4: Memory hardening | Process memory dumps | Medium | Container config | None |
| 5: Session hardening | Session hijacking | Low | None new | None |
