...
class Settings(BaseSettings):
    ...
    def validate_cross_field_dependencies(self) -> "Settings":
        ...
        Note: no field_validator reads approved_directory — it is Optional[Path]
        and may still be None at field-validation time.  The backfill at the
        bottom of this method is the only place it gets set from
        _approved_directories_cache, so the Optional type is safe until here.
        """
