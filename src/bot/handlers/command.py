...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (
        ...
        if manager is None:
            await update.message.reply_text(
                t("classic.start.misconfigured", lang),
                parse_mode="HTML",
            )
            return
        ...
        try:
            ...
            sync_section = t(
                "classic.start.sync_section", lang,
                created=sync_result.created, reused=sync_result.reused,
                renamed=sync_result.renamed, failed=sync_result.failed,
            )
            ...
            sync_section = t(
                "classic.start.sync_warning", lang,
                error=escape_html(str(e)),
            )

    welcome_message = (
        t("classic.welcome", lang, name=escape_html(user.first_name))
        + sync_section
    )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                t("btn.show_projects", lang), callback_data="action:show_projects"
            ),
            InlineKeyboardButton(t("btn.get_help", lang), callback_data="action:help"),
        ],
        [
            InlineKeyboardButton(t("btn.new_session", lang), callback_data="action:new_session"),
            InlineKeyboardButton(t("btn.check_status", lang), callback_data="action:status"),
        ],
    ]
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /help command."""
    lang = get_user_lang(context)
    await update.message.reply_text(t("classic.help", lang), parse_mode="HTML")
...
async def sync_threads(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not settings.enable_project_threads:
        await update.message.reply_text(
            t("classic.sync.disabled", lang), parse_mode="HTML"
        )
        return
    ...
    if not manager:
        await update.message.reply_text(
            t("classic.sync.not_init", lang), parse_mode="HTML"
        )
        return

    status_msg = await update.message.reply_text(
        t("classic.sync.syncing", lang), parse_mode="HTML"
    )

    if settings.project_threads_mode == "private":
        if not _is_private_chat(update):
            await status_msg.edit_text(
                t("classic.sync.private_error", lang),
                parse_mode="HTML",
            )
            return
        ...
        if settings.project_threads_chat_id is None:
            await status_msg.edit_text(
                t("classic.sync.group_no_chat_id", lang),
                parse_mode="HTML",
            )
            return
        if (
            ...
            await status_msg.edit_text(
                t("classic.sync.group_wrong_chat", lang),
                parse_mode="HTML",
            )
            return
    ...
    try:
        ...
        await status_msg.edit_text(
            t("classic.sync.complete", lang,
              created=result.created, reused=result.reused,
              renamed=result.renamed, reopened=result.reopened,
              closed=result.closed, deactivated=result.deactivated,
        ...
        await status_msg.edit_text(
            t("classic.sync.failed", lang, error=escape_html(str(e))),
            parse_mode="HTML",
        )
        if audit_logger:
            await audit_logger.log_command(user_id, "sync_threads", [], False)
...
async def new_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cleared_info = ""
    if old_session_id:
        cleared_info = t("classic.new.cleared", lang, session_id=old_session_id[:8])

    keyboard = [
        [
            InlineKeyboardButton(
                t("btn.start_coding", lang), callback_data="action:start_coding"
            ),
            InlineKeyboardButton(
                t("btn.change_project", lang), callback_data="action:show_projects"
            ),
        ],
        [
            InlineKeyboardButton(
                t("btn.quick_actions", lang), callback_data="action:quick_actions"
            ),
            InlineKeyboardButton(t("btn.help", lang), callback_data="action:help"),
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.message.reply_text(
        t("classic.new.message", lang, path=relative_path, cleared_info=cleared_info),
        parse_mode="HTML",
...
async def end_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(t("btn.status", lang), callback_data="action:status"),
            InlineKeyboardButton(t("btn.help", lang), callback_data="action:help"),
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    await update.message.reply_text(
        t("classic.end.ended", lang, path=relative_path),
        parse_mode="HTML",
        reply_markup=reply_markup,
    )
