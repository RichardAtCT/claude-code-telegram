...
async def handle_cd_callback(
    ...
    try:
        ...
        if security_validator:
            # new_path is already absolute (current_root / project_name), so
            # current_dir as the resolution base only affects relative inputs.
            # The downstream _is_within_any_directory still enforces the
            # approved-directory boundary, so there is no path-escape risk here.
