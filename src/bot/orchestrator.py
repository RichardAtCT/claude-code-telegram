...
class MessageOrchestrator:
    ...
    async def get_bot_commands(self) -> list:  # type: ignore[type-arg]
        ...
        if self.settings.agentic_mode:
            commands = [
                ...
                BotCommand("repo", "List all repos (global) / switch workspace"),
            ]
            if self.settings.enable_project_threads:
    ...
    async def agentic_repo(
        ...
        /repo          — list subdirectories across *all* approved roots with git indicators
        /repo <name>   — switch to that directory, resume session if available

        Note: /repo is global (shows all approved roots), while /ls and /projects
        are scoped to the current root. Use /repo to jump between roots.
        """
        args = update.message.text.split()[1:] if update.message.text else []
